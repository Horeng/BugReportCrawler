				...
				if(bugChild.getNodeType() ==Node.ELEMENT_NODE){
					if(bugChild.getNodeName().equals(String.valueOf("buginformation"))){
						...
						for(int k = 0; k<bugInfoChild.getLength(); k++){
							...
							if(bugInfo.getNodeName().equals(String.valueOf("attachment"))){
								ArrayList<Attachment> attachmentList = new ArrayList<Attachment>();
								Attachment attachment = new Attachment();								
								try{									
									System.out.println(bugInfo.getChildNodes().item(0).getNodeValue());
									attachment.setContent(bugInfo.getChildNodes().item(0).getNodeValue());
									attachmentList.add(attachment);
									bug.setAttachmentList((ArrayList<Attachment>) attachmentList.clone());
								}catch(Exception e){									
									bug.setAttachmentList((ArrayList<Attachment>) attachmentList.clone());
								}								
							}
