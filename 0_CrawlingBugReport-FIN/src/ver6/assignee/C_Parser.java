	...
	public boolean parse(int bugID) throws Exception
	{
		...
		try{
			...
			if(!initAssignee.equals(""))
				db.insertInitBugReport(bugID, bugAut.replace("'","."), prdName, compName, prodVersion, hw,initAssignee, assignee, openDate, modifiedDate, bugStatus, priority,severity, bugSum.replace("'","."), bugDes.replace("'","."));
